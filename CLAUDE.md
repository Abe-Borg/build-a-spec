...
- **Which release carries this is decided by the v1.21.0 tag, not by this
  ...
  on `master` with no release entry yet, which is true both ways.
- **The master merge resolved four docs conflicts.** CLAUDE.md: the two
  sections kept in merge order, the closeout's first. README: the redline
  section takes the closeout's "(in progress)" heading back (Phase 2 is
  still to come), and its intro says Phase 0 and the backend are in 1.21.0
  and the UI is on `master`. The plan's status line and the plans index:
  combined.
