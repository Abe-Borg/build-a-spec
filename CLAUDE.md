...
## Installing an update is gated like closing the window — implemented notes

Batch 2 of the 2026-09-02 program diagnosis. `POST /api/update/install`
downloads, verifies and SPAWNS THE INSTALLER, which closes the app — and it
had no gate at all: not against a streaming turn, a running research or
Final QC pass, a tutorial workspace, unsaved work, or a second click during
the download. The frontend buttons gated on `installing` alone, and the
success/failure chat bubble was pushed even mid-stream. No new endpoint, no
new SSE event, no new dep, no new env knob; one optional request body.

- **The gates are the window-close prompt's, in the same order the close
  path applies them.** `UpdateInstallRequest(acknowledge_unsaved=False)` is
  the body (a bodyless POST — every pre-existing caller — reads as
  all-default). Under the route: a per-app `threading.Lock` acquired
  NON-blocking → 409 `install_in_progress` (two clicks used to be two
  downloads into the same `.part`); the platform 400 as before; the
  workspace scope → 409 `tutorial_active` (the user's project is parked
  behind the tour); then under `session_state_guard()` ONE read of
  `busy_reasons` + `has_unsaved_progress` → 409 `workspace_busy` naming
  what is running, then 409 `unsaved_progress` unless acknowledged. Every
  refusal goes through `_coded_error_response` and leaves an `update`
  app_event carrying the code. Busy is checked before unsaved because a
  streaming turn is both, and "wait" is the more useful answer.
- **`sessions.get_workspace()` is called BEFORE the guard, never under it.**
  The manager lock and the turn-state lock are taken the other way round by
...
- **The lock is released on EVERY exit, success included, by design.**
  ...
  case where a second click is exactly right. What the lock exists for is
  the window DURING a download; a deliberate later click is a new attempt.
...
- **`unsaved_progress` is a prompt, not an error.** `lib/api.ts`'s
  ...
  turns that one code into `setSaveGate({kind: "install-update"})` — the
  SAME `CloseDialog` New session / Open project / template / brief already
  use, with install copy — and never sets `installError` or a bubble for
  it. Save runs `saveProjectFile()` and then `performInstall(true)` only
  when a file was written (a cancelled Save dialog keeps the session, the
  gate's standing rule); "Install without saving" is `performInstall(true)`
  directly; Cancel does nothing. The acknowledgement is the client's
  promise that the prompt was shown — the server cannot know, which is why
  it is a flag and not a second endpoint.
- **The chat bubble is pushed only when no turn is streaming.** The old
  diagnosis blamed `appendToLast`; the actual mechanism was a fresh bubble
  pushed via `setMessages` AFTER the streaming one, so the reply's next
...
- **Disabled, with the reason on hover, takes three things together.** A
  ...
  `Tip`'s span, which holds the server's sentence, and without it on the
  bare button, which holds nothing. All three are pinned. `Tip` renders an
  `inline-flex` span, which in a block menu shrinks to its text (258 of
  286 px, measured) unless it takes `w-full` — and then the button, its
  hover highlight and its click target span the row like every other item.
  (`align-top` was tried and measured to change nothing; an
  `inline-flex`'s baseline is its text's, so there is no line-box gap to
  remove.)
...
- **Which release carries this is decided by the v1.21.0 tag, not by this
  PR.** The project-workspace closeout (PR #188) merged while this was in
  review. Its 1.21.0 entry was written when only the backend had merged, so
  it leaves the redline out. v1.21.0 was still untagged when this was
  written, and the closeout's tag step tags `master` as it stands. Tagged
  before this merges, 1.21.0 ships none of this UI, and the Phase 1 draft
  waits for the next release. Tagged after, 1.21.0 ships the menu item —
  unannounced, unless before the tag its entry (not frozen until tagged)
  takes the Phase 1 draft. The closeout's statements that 1.21.0 reaches the
  redline only through the API then need correcting too; the plan's
  "Phase 1 (UI PR) — as built" note lists them. Nothing here claims either
  order: README, the plan's status line and the plans index say the UI is
  on `master` with no release entry yet, which is true both ways. PR #189
  (compaction Phase 3) merged first and raises the same question for its
  own work; tagging the closeout merge, `a273ab7`, instead of `master`
  keeps both out of 1.21.0.
- **Two master merges, both docs-only.** The first (the closeout, PR #188)
  conflicted in four files. CLAUDE.md: the two sections kept in merge
  order, the closeout's first. README: the redline section takes the
  closeout's "(in progress)" heading back (Phase 2 is still to come), and
  its intro says Phase 0 and the backend are in 1.21.0 and the UI is on
...
- **One lint pass per committed version.** `SessionState.document_lint`
  ...
  readiness poll and the next turn's context — two for an imported master
  with preserved header or footer lines, because the event passes none
  (next bullet).
- **Each caller keeps exactly its old arguments**, so no finding moved
  anywhere, including one pre-existing oddity left alone on purpose: the
  post-commit `lint` event passes no `preserved_chrome`, so for an imported
  master whose preserved footer names another section the
  `stale_document_identifier` finding blinks out of the Issues drawer until
  the refresh after `turn_complete` restores it. Fixing it changes that
  event's findings, so it is a suggested follow-up, not part of this
  change; the memo keeps both input sets for a version apart meanwhile.
