...
change (`word_load_check_failed`). A moved provision holding custom XML is
never shown as a native move (`moves_fallback` counts it as `markup`): it
keeps the deletion-plus-insertion rendering described here. A Word-saved
master never holds custom XML, because Word strips it on save. Other
producers still write it.
