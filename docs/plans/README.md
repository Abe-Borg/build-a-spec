...
its own numbering level. Another (the Phase 0 follow-up) fixed the four
things Phase 0 left behind: refs that counted preserved blocks, the styled
export numbering one, a non-spec import's placeholder headings in the
formatted export and the redline, and a template clone's pending revisions.
Neither follow-up has a release entry. **Phase 2 lands as two PRs** (a deviation: the plan
