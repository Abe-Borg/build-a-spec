...
- Status: complete
- Commit/PR: branch `claude/phase-6-concurrency-responsiveness-xncqik`
- Tests: 4 new, each reverted in place to confirm it goes red.
  ...
  version it committed, not a later one. `tests/test_app.py`: `/api/doc`
  reads one state, not a mixture of two. Full suite 1373 passed / 9
  skipped (no new skips); `npm test` 193; `npm run build` clean.
- Deviations:
