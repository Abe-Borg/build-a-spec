...
| WL-1 | Chunk 3 | Check that the batch reads the warm lead | done | PR #227 | — |
...
- [x] WL-1.3 — per-seat first-iteration read share; unmeasured seats counted — evidence: `test_a_seat_read_the_prefix_at_95_percent_and_not_below`, `test_what_cannot_be_read_is_unmeasured_and_nothing_raises`, `test_a_searched_seat_is_measured_from_its_reported_first_iteration`, `test_a_paused_seat_is_measured_from_its_first_response`, `test_a_web_tooled_lineage_that_searched_without_iterations_is_unmeasured`
...
- [x] WL-1.8 — `tests/test_cost_checks_warm_lead.py` and the frontend cases — evidence: `tests/test_cost_checks_warm_lead.py` (41 tests, 43 with parametrized cases); `frontend/tests/costChecks.test.ts` (17 tests, 8 of them new), already in `frontend/package.json`'s test script
...
- [x] WL-1.11 — revert matrix recorded — evidence: the plan's WL-1 As built, "Revert matrix" (47 rows, 47 red)
