  ...
  return (
    <div className="space-y-4">
      ...
      <Recipe
        ...
        steps={[
          "Before you leave a finished section, check the Project facts panel: the next section starts with the facts recorded there, never with this conversation. Harvest facts… proposes the ones this section settled but nobody recorded — it reads the conversation since the last harvest, the provisions with their statuses and your Final QC dismissal reasons, and shows each proposal with the line it rests on. You accept, edit or reject each one; nothing is recorded until you do, and a proposal citing a source that does not exist cannot be recorded. It is one paid call, run only when you press Run; Next section and the Export menu say how many replies are waiting and offer it, but never run it.",
          "In the finished section, press Next section → in the panel: it offers to save, then opens the next section of the same project straight from this one — pick it from the module's catalog (sections already drafted are greyed), type a header, or leave it unnamed for an office-master import. Everything below happens without a file.",
          "Keep the project in one folder — its project brief beside each section's saved file — and the desktop app finds it: the Project panel above Project facts names the folder, lists every section the project has drafted, marks the one you are in, and opens any other by its number in one click after offering to save. The first save of a section started with Next section → opens in that folder, and a section saved before its first brief export joins the folder at its next Save. Nothing about the folder is written into any file, so it can be moved or shared whole.",
          "The file route still exists for a section drafted on another day or another machine: Export → Export project brief. The .basproject carries the project profile and type, the recorded edition overrides, every research round, the attached reference documents (their full text), the established project facts, and a registry of the sections drafted so far — not the chat, the document, figures, or the Final QC report.",
      ...
      <Recipe
        ...
        steps={[
          "After importing an office master and working on it here, choose Export → Redline on your original (tracked changes). It is the Word file you imported with every change since the import as a Word tracked change by Build-a-Spec. Apart from Build-a-Spec's comments, every part of the file outside the document body is your upload's, byte for byte.",
          "A change with a recorded basis carries a Word comment from Build-a-Spec saying what it rests on: the research finding (its requirement, authority and code reference), the attached document, or the Final QC fix — with clickable links to the sources. The comments stay through Accept All and Reject All, and your own comments are kept. The file may go to a client, so read them first; to remove them, show only Build-a-Spec's markup (Review → Show Markup → Specific People) and delete the comments shown.",
          "In Word, Accept All gives exactly what Export Word (keeps your formatting) produces and Reject All gives your original back, Build-a-Spec's comments aside. The app checks both on every file before it hands it over, and refuses with the reason rather than deliver one that fails.",
          ...
          "In a master with typed letters, a provision relettered by an insert above it shows the letter change; a Word-numbered master renumbers itself. A provision you moved without changing it shows as Word's own Moved marks — moved from where it was, moved to where it is, paired as one move in Word's Reviewing Pane; one that also changed (in a typed-letter master, a new letter counts) shows as a deletion where it was and an insertion where it is. One limit: a moved provision keeps its bookmarks at its new position, so Reject All does not restore them where it was.",
